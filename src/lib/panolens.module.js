...
Reticle.prototype = Object.assign(Object.create(Sprite.prototype), {
  ...
  ripple: function() {
    ...
    const update = () => {
      ...
      context.fillStyle = `rgba(${color.r * 255}, ${color.g * 255}, ${color.b *
        255}, ${opacity})`;
      context.fill();
      context.closePath();
...
function ImagePanorama(image, _geometry, _material) {
  ...
  const material =
    _material || new MeshBasicMaterial({ opacity: 0, transparent: true });
...
function DeviceOrientationControls(camera, domElement) {
  ...
  this.update = function(ignoreUpdate) {
    ...
    var alpha = scope.deviceOrientation.alpha
      ? Math$1.degToRad(scope.deviceOrientation.alpha) + scope.alphaOffsetAngle
      : 0; // Z
    var beta = scope.deviceOrientation.beta
      ? Math$1.degToRad(scope.deviceOrientation.beta)
      : 0; // X'
    var gamma = scope.deviceOrientation.gamma
      ? Math$1.degToRad(scope.deviceOrientation.gamma)
      : 0; // Y''
    var orient = scope.screenOrientation
      ? Math$1.degToRad(scope.screenOrientation)
      : 0; // O
...
function Viewer(options) {
  ...
  options.autoReticleSelect =
    options.autoReticleSelect !== undefined ? options.autoReticleSelect : true;
  ...
  this.renderer =
    options.renderer || new WebGLRenderer({ alpha: true, antialias: false });
...
Viewer.prototype = Object.assign(Object.create(EventDispatcher.prototype), {
  ...
  tweenControlCenter: function(vector, duration, easing) {
    ...
    vptc = this.panorama
      ...
      .sub(this.camera.getWorldPosition(new Vector3()));
    ...
    ha = ha < -Math.PI ? ha + 2 * Math.PI : ha;
    va = Math.abs(
      cvv.angleTo(chv) + (cvv.y * vv.y <= 0 ? vv.angleTo(hv) : -vv.angleTo(hv))
    );
    va *= vv.y < cvv.y ? 1 : -1;
  ...
  tweenControlCenterByObject: function(object, duration, easing) {
    ...
    if (isUnderScalePlaceHolder) {
      ...
      this.tweenControlCenter(
        object.getWorldPosition(new Vector3()).multiply(invertXVector),
        duration,
        easing
      );
    } else {
...
if (REVISION$1 != THREE_REVISION) {
  console.warn(
    `three.js version is not matched. Please consider use the target revision ${THREE_REVISION}`
  );
}
